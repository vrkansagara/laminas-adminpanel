...
InterpreterManager.prototype.help = function(fn) {
  ...
  var url = "assets/js/vendor/MochiKit/doc/html/" + base.join("/") + ".html";
  var d = doXHR(url, { mimeType: "text/xml" });
  d.addCallback(function(req) {
    var els = getElementsByTagAndClassName("a", "mochidef", req.responseXML);
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (idx != -1 && href.substring(idx) == match) {
        writeln(A({ href: url + match, target: "_blank" }, scrapeText(elem)));
        return;
      }
    }
